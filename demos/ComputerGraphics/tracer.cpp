...
public:
    ...
    double radius;
    Sphere() = default;
    Sphere(double r, Vec p, Vec e, Vec c, MaterialType m)
        : emission(e), color(c), center(p), material(m), radius(r) {}
     ...
     */
    Vec normal(const Vec& point) const {
        Vec gradient_result{};
 ...
 */
Vec radiance(const Scene& scene, Ray ray) {
    ...
    for (int depth = 0; ; ++depth) {
        ...
        const auto& obj = scene[hit.object_id];
 ...
 */
void render(int w, int h, int samples, const Ray& cam, const Scene& scene, Vec* image) {
 ...
 */
double loss_function(int w, int h, int samples, const Ray& cam, const Scene& scene, const Vec *target_image) {
...
int main(int argc, char* argv[]) {
    ...
    // const std::vector<Vec>& target_image = image;
    auto grad_func = clad::gradient(loss_function, "cam");
    grad_func.dump();

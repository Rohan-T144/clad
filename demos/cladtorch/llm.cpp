...
*/
#include <vector> // Used for safely managing a temporary array
#include <assert.h>
#include <ctype.h>
#include <math.h>
#include <stdint.h>
...
struct GPT2 {
  ...
  GPT2(const char* checkpoint_path) : config(checkpoint_path), params(config) {
    FILE* model_file = fopenCheck(checkpoint_path, "rb");
    int model_header[256];
    freadCheck(model_header, sizeof(int), 256, model_file);
    printf("[GPT-2]\n");
    ...
    num_parameters = 0;
    for (size_t i = 0; i < NUM_PARAMETER_TENSORS; i++)
      num_parameters += this->params.sizes[i];
    printf("num_parameters: %zu\n", num_parameters);
  
    // read in all the parameters from file
    freadCheck(this->params.memory, sizeof(float), num_parameters, model_file);
    fcloseCheck(model_file);

...
struct Layer1 {
  ...
  [[nodiscard]] FTensor forward(const FTensor& input) const {
    return gelu(matmul(W, input) + b); // Apply GELU activation
  }
...
struct Layer2 {
  ...
  FTensor forward(const FTensor& hidden) const { return matmul(W, hidden) + b; }

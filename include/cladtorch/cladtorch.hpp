...
template <typename T, size_t R, size_t C> Tensor<T, R> mat_vec_mul(const Tensor<T, R, C>& m, const Tensor<T, C>& v) {
  Tensor<T, R> r;
  kernels::mat_vec_mul_kernel(m._data, v._data, r._data, R, C);
  return r;
}
template <typename T, size_t... Dims> Tensor<T, Dims...> apply_gelu(const Tensor<T, Dims...>& in) {
  Tensor<T, Dims...> r;
